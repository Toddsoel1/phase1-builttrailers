...
export async function createPO(partId, qty, userId) {
  ...
  await q(`INSERT INTO purchase_order(id,vendor_id,part_id,qty,unit_cost,placed,eta,status,created_by)
           VALUES ($1,$2,$3,$4,$5,CURRENT_DATE,$6,'Open',$7)`,
    [id, p.vendor_id, partId, qty, p.cost, eta.toISOString().slice(0, 10), userId || null]);
  await q('INSERT INTO audit_log(user_id,action,detail) VALUES ($1,$2,$3)', [userId || null, 'po.create', `${id} ${partId} x${qty}`]);
  return id;
}
