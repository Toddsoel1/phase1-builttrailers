// Built Trailers' real dealer network, seeded into the customer table for the public locator.
// Coordinates are pre-geocoded (OpenStreetMap) and baked in, so seeding is instant and offline.
// ensureDealers() applies the list ONCE (guarded by an app_config flag) at server startup — it
// upserts every dealer that first time (so "updated or added" both work), then never runs again,
// so any later office edits to a dealer's address/coordinates are preserved.
import { q, one } from './db.js';

// name, street, city, state, zip, phone ('' = unknown), lat, lng (null = set by hand)
...
// Upsert one dealer by name (insert new, or update the address/phone/coords of an existing one).
export async function upsertDealer([name, address, city, state, zip, phone, lat, lng], i = 0) {
  ...
  await q(`INSERT INTO customer(id,name,kind,active,address,city,state,zip,phone,lat,lng)
    ...
    ['cd' + i + '-' + Date.now().toString(36), name, address, city, state, zip, phone || null, lat, lng]);
  return 'added';
}

// Runs once at startup. After it applies, the flag stops it re-running so office edits survive.
export async function ensureDealers() {
  if (await one(`SELECT value FROM app_config WHERE key='dealers_seeded_v1'`)) return { skipped: true };
  let added = 0, updated = 0;
  for (let i = 0; i < DEALERS.length; i++) (await upsertDealer(DEALERS[i], i)) === 'added' ? added++ : updated++;
  await q(`INSERT INTO app_config(key,value) VALUES('dealers_seeded_v1','1') ON CONFLICT(key) DO UPDATE SET value='1'`);
